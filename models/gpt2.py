...
class GPT2Model(GPTPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    # Token Embeddings
    inputs_embeds = self.word_embedding(input_ids)  # Shape: [batch_size, seq_len, hidden_size]

    # Positional Embeddings
    pos_ids = self.position_ids[:, :seq_length]
    pos_embeds = self.pos_embedding(pos_ids)  # Shape: [1, seq_len, hidden_size]
  ...
  def hidden_state_to_token(self, hidden_state):
    ...
    return self.embed_dropout(hidden_state) * self.word_embedding.weight.T
